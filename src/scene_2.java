...
public class scene_2 extends JPanel implements Runnable{
	/**
	 * 
	 */
	...
	private final static int width2=1438;     //背景尺寸
	private final static int height2=932;
	private static boolean run2=false;         //线程状态
	private boolean up,down,left,right;     //移动状态
	private npc2 npc; //该场景内其他鱼的资料状态
	...
	public scene_2() {
		...
		ImageIcon tmp = new ImageIcon(this.getClass().getResource("image/bg2.jpg"));
		background2 = tmp.getImage(); 
		//初始位置x在左边，y在场景的中央
	...
	public void paint(Graphics g){    
		...
		//判断是否进入传送门
		if(x>1320&&x<1381&&y>198&&y<240){
			MainFrame.toScene1(2);
		}
		if(x>1207&&x<1256&&y>858&&y<903){
			MainFrame.toScene3(2);
		}
		Image offscreen=createImage(getWidth2(),getHeight2());       
		...
		if(x>=Fw/2&&x<=getWidth2()-Fw/2&&y>=Fh/2&&y<=getHeight2()-Fh/2)   
		...
		else if(x<Fw/2&&y>=Fh/2&&y<=getHeight2()-Fh/2)
			g.drawImage(offscreen, 0, Fh/2-y,null);
		else if(x<Fw/2&&y>getHeight2()-Fh/2)
			g.drawImage(offscreen, 0, Fh-getHeight2(),null);
		else if(x>=Fw/2&&x<=getWidth2()-Fw/2&&y<Fh/2)
			g.drawImage(offscreen, Fw/2-x, 0,null);
		else if(x>=Fw/2&&x<=getWidth2()-Fw/2&&y>getHeight2()-Fh/2)
			g.drawImage(offscreen, Fw/2-x, Fh-getHeight2(),null);
		else if(x>getWidth2()-Fw/2&&y<Fh/2)
		    g.drawImage(offscreen, Fw-getWidth2(), 0,null);
		else if(x>getWidth2()-Fw/2&&y>=Fh/2&&y<=getHeight2()-Fh/2)
			g.drawImage(offscreen, Fw-getWidth2(), Fh/2-y,null);
		else if(x>getWidth2()-Fw/2&&y>getHeight2()-Fh/2)
		    g.drawImage(offscreen, Fw-getWidth2(), Fh-getHeight2(),null);
